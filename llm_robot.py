...
# WebSocket URLs
VIDEO_URL = "ws://192.168.0.55:4000/video-stream"
AUDIO_URL = "ws://192.168.0.55:4000/audio-stream"
...
def get_distance_to_object():
    ##Logic to get distance from ultrasonic sensor from ESP32 through ESPNOW 
    ...
    while True:
        ...
        if command == "MOVE_FORWARD" and in_scene:
            ...
            if distance > 2.0:
                ...
                ##Logic to send to ESP32 through ESPNOW to move forward


                time.sleep(1)  # Simulate forward movement
            else:
                ##Logic to send to ESP32 through ESPNOW to stop

                print("Reached the object. Stopping.")
                break
        elif command == "ROTATE":
            print(f"Rotating by {rotate_degree} degrees to search for the object.")
            ##Logic to send to ESP32 through ESPNOW to rotate
